...
const AddTransaction = () => {
  ...
  return (
    <div className="w-full h-full flex flex-col overflow-hidden relative select-none bg-[#141115] overscroll-none">
      ...
      <div className="px-4 pt-1.5 pb-1 shrink-0 z-20">
        <div className="flex bg-black/20 backdrop-blur-[10px] border border-white/5 p-0.5 sm:p-1 rounded-full shadow-inner relative max-w-[340px] mx-auto w-full h-[36px] sm:h-[38px] items-center">
          {[
            ...
            return (
              <button
                ...
                className="relative flex-1 h-full rounded-full text-[12.5px] sm:text-[13.5px] font-semibold transition-colors duration-200 flex items-center justify-center z-10 active:scale-[0.97]"
              >
                {isActive && (
                  <motion.div
      ...
      <div className="w-full flex-1 min-h-0 bg-[#2B2321]/30 backdrop-blur-[32px] rounded-t-[28px] sm:rounded-t-[32px] border-t border-white/10 px-4 sm:px-5 pt-2 sm:pt-2.5 pb-[74px] sm:pb-[80px] flex flex-col overflow-hidden relative z-10">
        ...
        <form onSubmit={handleSubmit} className="w-full flex-1 min-h-0 flex flex-col justify-between">
          ...
          <div className="flex flex-col items-center justify-center shrink-0">
            <label className="text-[11px] sm:text-[12px] font-medium text-white/70 tracking-wide mb-0.5 text-center">
              {t('addTransaction.amount')}
            </label>
            <div className="flex items-center justify-center gap-2 relative w-full h-[46px] sm:h-[52px]">
              <button
                type="button"
                onClick={() => setShowCalculator(true)}
                className="absolute start-1 w-8 h-8 sm:w-9 sm:h-9 rounded-full bg-black/20 backdrop-blur-[10px] border border-white/5 shadow-inner text-white/60 hover:text-white hover:bg-white/10 flex items-center justify-center transition-all active:scale-95"
              ...
              <input
                ...
                placeholder="0"
                className="bg-transparent text-center text-[30px] sm:text-[36px] font-bold text-white focus:outline-none w-[auto] min-w-[70px] max-w-[220px] placeholder-white/25 tracking-tight leading-none h-full tabular-nums"
                style={{ caretColor: type === 'expense' ? '#FF5555' : type === 'income' ? '#34C759' : '#007AFF' }}
              />
              <span className="text-[12px] sm:text-[13px] text-white/80 font-semibold self-end mb-0.5">
                {t('nav.currency')}
              </span>
            </div>
            <div className="w-[50%] max-w-[150px] h-[1px] bg-gradient-to-r from-transparent via-white/15 to-transparent mx-auto mt-0.5" />
          </div>

          {/* 2. Date Segmented Control */}
          <div className="shrink-0">
            <label className="block text-[11px] sm:text-[12px] font-medium text-white/70 mb-0.5 px-1">
              {t('addTransaction.date')}
            </label>
            <div className="flex bg-black/20 backdrop-blur-[10px] border border-white/5 p-0.5 sm:p-1 rounded-full shadow-inner relative h-[36px] sm:h-[38px] items-center">
              {[
          ...
          <div className="shrink-0">
            ...
            <input
              ...
              className="w-full h-[38px] sm:h-[40px] bg-black/20 backdrop-blur-[10px] border border-white/5 shadow-inner rounded-full px-3.5 sm:px-4 text-[13px] sm:text-[14px] text-white placeholder-white/35 focus:outline-none focus:border-[#8D6346] focus:bg-black/30 transition-all hover:bg-white/5"
            />
          </div>
          ...
          {type === 'transfer' ? (
            <>
              <div className="shrink-0">
                <label className="block text-[11px] sm:text-[12px] font-medium text-white/70 mb-0.5 px-1 truncate">
                ...
                <CustomSelect
                  ...
                  buttonClassName="w-full h-[38px] sm:h-[40px] bg-black/20 backdrop-blur-[10px] border border-white/5 shadow-inner rounded-full px-3.5 sm:px-4 text-[13px] sm:text-[14px] font-medium text-white flex items-center justify-between transition-all hover:bg-white/5 focus:outline-none focus:border-[#8D6346]"
                />
              </div>
              <div className="shrink-0">
                <label className="block text-[11px] sm:text-[12px] font-medium text-white/70 mb-0.5 px-1 truncate">
          ...
          <div className="flex flex-col gap-1.5 sm:gap-2 shrink-0 mb-0.5 sm:mb-1">
            {/* 6. Recurring Settings Button */}
            {recurringSettings.repeatType === 'never' ? (
              <button
                type="button"
                onClick={() => setIsRecurringModalOpen(true)}
                className="w-full h-[38px] sm:h-[40px] flex items-center justify-center gap-2 border border-dashed border-white/15 hover:border-white/25 rounded-full bg-black/20 backdrop-blur-[10px] shadow-inner transition-all duration-200 active:scale-[0.98]"
              >
                <Repeat className="w-3.5 h-3.5 text-white/60" />
                <span className="text-[12.5px] sm:text-[13.5px] font-semibold text-white/75">{t('recurring.addBtn')}</span>
              </button>
            ) : (
              <div className="w-full h-[38px] sm:h-[40px] flex items-center justify-between px-3.5 sm:px-4 border border-dashed border-[#8D6346]/40 rounded-full bg-[#8D6346]/10 backdrop-blur-[10px] shadow-inner transition-all duration-200">
                <div className="flex items-center gap-2">
                  <Repeat className="w-3.5 h-3.5 text-[#E8C5A8]" />
                  <span className="text-[12.5px] sm:text-[13.5px] font-semibold text-[#E8C5A8]">{t('recurring.settings')}</span>
                </div>
                <button
                  type="button"
                  onClick={() => setIsRecurringModalOpen(true)}
                  className="px-3 py-0.5 bg-[#8D6346] hover:bg-[#734e35] rounded-full text-[11px] font-medium text-white transition-colors"
                >
                  {t('recurring.edit')}
                </button>
              </div>
            ...
            <motion.button
              whileTap={{ scale: 0.96 }}
              type="submit"
              className="w-full h-[40px] sm:h-[44px] rounded-full font-bold text-[14px] sm:text-[15px] text-white shadow-inner transition-colors duration-200 bg-[#8D6346]/25 backdrop-blur-[10px] border border-[#8D6346]/40 hover:bg-[#8D6346]/35 flex items-center justify-center gap-2"
            >
