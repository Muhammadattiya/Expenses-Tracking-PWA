...
const onboardingSteps = [
  {
    ...
    defaultTitle: "Your money, finally working for you.",
    defaultDesc: "Finova tracks your spending, understands your habits, and turns your financial data into clear, personalized insights. Predict what's next, explore what-if scenarios, and make every financial decision with confidence."
  },
  {
    id: 2,
...
export default function Onboarding() {
  ...
  // Using ref for registered step action prevents accidental immediate execution by React state updaters
  const nextActionRef = useRef(null);
  const registerNext = useCallback((fn) => {
  ...
  const handleNext = async () => {
    if (nextActionRef.current) {
      setLoading(true);
      try {
        let action = nextActionRef.current;
        ...
        console.error('Next action execution failed:', error);
        setLoading(false);
        return;
      }
      setLoading(false);
  ...
  return (
    <div
      dir={isRTL ? 'rtl' : 'ltr'}
      className="w-full relative min-h-screen text-white flex flex-col justify-between selection:bg-[#8D6346]/40 overflow-x-hidden"
    >
      {/* Universal Ambient Copper Glow (Exact Theme from Dashboard.jsx) */}
      <AmbientBackground variant="dashboard" />

      {/* Top Sticky Header Bar */}
      <header className="sticky top-0 z-30 w-full backdrop-blur-xl bg-[#141115]/80 border-b border-white/[0.08] px-4 pt-[max(0.75rem,env(safe-area-inset-top))] pb-3 transition-colors">
        <div className="w-full max-w-md mx-auto flex items-center justify-between">
          ...
          {currentStep > 0 ? (
            <motion.button
              ...
              className="size-10 flex items-center justify-center rounded-xl bg-white/5 hover:bg-white/10 active:bg-white/15 border border-white/10 text-white/90 shadow-sm transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#8D6346]"
            >
          ...
          {/* Localized Step Counter Badge (No neutral slash bidi reverse!) */}
          <div className="px-4 py-1.5 rounded-full bg-black/40 backdrop-blur-md border border-white/10 text-xs font-bold text-white/80 font-['Exo_2'] tracking-wide shadow-inner">
            {isRTL 
              ? `الخطوة ${currentStep + 1} من ${onboardingSteps.length}` 
          ...
          {currentStep < onboardingSteps.length - 1 ? (
            <motion.button
              ...
              whileTap={{ scale: 0.95 }}
              onClick={handleSkip}
              className="h-10 px-3.5 flex items-center justify-center rounded-xl bg-transparent hover:bg-white/5 text-white/60 hover:text-white/90 text-xs font-semibold font-['Exo_2'] transition-all"
            >
      ...
      {/* Main Flow Canvas — Natural Document Scrolling (Matches Dashboard.jsx) */}
      <main className="flex-1 w-full max-w-md mx-auto px-4 py-6 flex flex-col justify-center items-center">
      ...
      {/* Bottom Sticky Action Deck — Solid Anchor with Primary CTA & Progress */}
      <footer className="sticky bottom-0 z-30 w-full backdrop-blur-2xl bg-[#141115]/85 border-t border-white/10 px-5 pt-3 pb-[max(1.25rem,env(safe-area-inset-bottom))] flex flex-col items-center gap-3">
        <div className="w-full max-w-md mx-auto flex flex-col items-center gap-3">
          {/* Primary CTA Button (Comfortable 56px Thumb Zone Tap) */}
          <motion.button
            whileTap={{ scale: 0.98 }}
            ...
            onClick={handleNext}
            disabled={loading}
            className="w-full h-14 rounded-2xl bg-gradient-to-r from-[#8D6346] via-[#9E7252] to-[#A47553] hover:brightness-110 active:scale-[0.98] border border-white/20 shadow-[0_8px_25px_rgba(141,99,70,0.45),inset_0_1px_1px_rgba(255,255,255,0.3)] transition-all flex items-center justify-center gap-2 cursor-pointer touch-manipulation focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#8D6346]"
          >
            {loading ? (
              <Loader2 size={20} className="animate-spin text-white" />
            ) : (
              <span className="font-['Exo_2'] font-bold text-base text-white tracking-wide">
                {currentStep === onboardingSteps.length - 1
                  ? (isRTL ? 'ابدأ استخدام فينوڤا' : 'Get Started with Finova')
          ...
          <div
            role="group"
            aria-label="Progress"
            className="flex items-center justify-center w-full max-w-[280px] gap-1.5"
          >
            {onboardingSteps.map((_, idx) => (
              idx === currentStep ? (
                <motion.div
                  ...
                  className="h-2 w-6 rounded-full bg-[#8D6346] shadow-[0_2px_8px_rgba(141,99,70,0.6)]"
                />
              ) : (
                <div
                  key={`dot-${idx}`}
