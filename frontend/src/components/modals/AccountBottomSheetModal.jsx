...
export default function AccountBottomSheetModal({
  isOpen,
  onClose,
  accounts = [],
  selectedId,
      ...
      >
        ...
        >
          ...
          <div className="flex items-center justify-between w-full mb-4 pb-2 border-b border-white/10">
            <div>
              <h3 id="account-modal-title" className="text-white font-bold text-lg tracking-wide">
                {title || t('addTransaction.account')}
              </h3>
              <p className="text-white/50 text-xs mt-0.5">
                {filteredAccounts.length} {lang === 'ar' ? 'حساب' : 'accounts'}
              </p>
            </div>
            <button
              type="button"
          ...
          {/* Accounts Grid / List */}
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 sm:gap-3 overflow-y-auto max-h-[55vh] py-1 pe-1 custom-scrollbar">
            {filteredAccounts.length === 0 ? (
              ...
              filteredAccounts.map((acc) => {
                const IconComponent = getIconComponent(acc.icon, 'Wallet');
                const isSelected = selectedId === acc._id;
                const accColor = acc.color || '#8D6346';
                ...
                return (
                  <motion.button
                    ...
                    className={`relative flex items-center justify-between p-3.5 rounded-2xl border transition-all duration-200 group text-start ${
                      isSelected
                        ? 'bg-[#8D6346]/25 border-[#8D6346] shadow-[0_0_16px_rgba(141,99,70,0.3)]'
                  ...
                  >
                    <div className="flex items-center gap-3 min-w-0">
                      <div 
                        ...
                        style={{ 
                          backgroundColor: isSelected ? `${accColor}33` : 'rgba(255,255,255,0.08)',
                          color: isSelected ? accColor : (accColor || 'rgba(255,255,255,0.85)')
                        }}
                      >
                      ...
                      <div className="min-w-0 flex flex-col">
                        ...
                        {formattedBal !== null && (
                          <div className="flex items-center gap-1 mt-0.5">
                            <span className="text-xs font-semibold text-[#E8C5A8] tabular-nums tracking-tight">
                              {formattedBal}
                            </span>
                    ...
                    {isSelected && (
                      <div className="w-5 h-5 rounded-full bg-[#8D6346] text-white flex items-center justify-center shrink-0 shadow-sm ms-2">
