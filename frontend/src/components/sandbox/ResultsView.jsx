...
const MetricCard = ({ label, before, after, diff, money, invertColors = false }) => {
  ...
  return (
    <div className="liquidglass rounded-[2rem] p-5 hover:border-white/20 transition-colors">
      <p className="text-sm font-medium text-white/50 mb-4">{label}</p>
      
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <p className="text-xs text-white/50 uppercase tracking-wider mb-1">Current</p>
          <p className="text-xl font-bold text-white/90 tabular-nums">{money(before)}</p>
        </div>
        
        <ArrowRight className="w-5 h-5 text-white/50 hidden sm:block opacity-50" />
        
        <div>
          <p className="text-xs text-white/50 uppercase tracking-wider mb-1">Simulated</p>
          <p className="text-xl font-bold text-white tabular-nums">{money(after)}</p>
        </div>
        
        <div className="sm:text-right">
          <p className="text-xs text-white/50 uppercase tracking-wider mb-1">Difference</p>
...
export default function ResultsView({ before, after, difference, money }) {
  return (
    <div className="space-y-4">
      <h3 className="text-lg font-bold text-white/90 px-2">Before & After Comparison</h3>
      ...
      <div className="pt-4">
        <h3 className="text-lg font-bold text-white/90 px-2 mb-4">Accounts Impact</h3>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {before.accounts.map(bAcc => {
            ...
            return (
              <div key={bAcc._id} className={`liquidglass ${isAffected ? 'border-[#8D6346]/50' : 'border-white/5'} rounded-2xl p-4 transition-colors`}>
                <div className="flex items-center gap-2 mb-3">
                  <Wallet className={`w-4 h-4 ${isAffected ? 'text-[#8D6346]' : 'text-white/50'}`} />
                  <p className="font-bold text-white/90 truncate">{bAcc.name}</p>
                </div>
                <div className="flex items-center justify-between">
                  <p className="text-sm text-white/50 tabular-nums">{money(bAcc.balance)}</p>
                  <ArrowRight className="w-4 h-4 text-white/50 opacity-50" />
                  <p className={`text-sm font-bold tabular-nums ${isAffected ? 'text-white' : 'text-white/50'}`}>{money(aAcc?.balance || 0)}</p>
                </div>
                {isAffected && (
