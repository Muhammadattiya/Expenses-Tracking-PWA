  ...
  return (
    <div className="w-full flex justify-center mt-2 md:mt-4 mb-6 md:mb-8">
      <div 
        ...
        className="flex gap-2 overflow-x-auto hide-scrollbar scrollbar-hide no-scrollbar relative p-[5px] liquidglass border border-white/5 shadow-inner rounded-[85px] w-full lg:w-auto overscroll-x-contain scroll-smooth px-3 lg:px-[5px] [mask-image:linear-gradient(to_right,transparent,black_16px,black_calc(100%-16px),transparent)] rtl:[mask-image:linear-gradient(to_left,transparent,black_16px,black_calc(100%-16px),transparent)] lg:[mask-image:none] [&::-webkit-scrollbar]:hidden"
      ...
      >
        {tabs.map((tab, idx) => (
          <motion.button
            ...
            className={`relative flex-1 lg:flex-none min-w-[110px] sm:min-w-[120px] min-h-[44px] py-3 px-3.5 sm:px-4 text-xs sm:text-sm font-bold ltr:tracking-wide rtl:tracking-normal rounded-[80px] transition-colors duration-300 z-10 flex items-center justify-center whitespace-nowrap outline-none focus-visible:ring-2 focus-visible:ring-[#E8C5A8]/70 ${activeTab === tab.id ? 'text-[#E8C5A8] drop-shadow-sm' : 'text-white/50 hover:text-white'}`}
          >
            {activeTab === tab.id && (
              <motion.div
