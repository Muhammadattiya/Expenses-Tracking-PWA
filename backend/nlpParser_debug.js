...
function extractIntent(text, options = { fallbackPrefix: false }) {
   ...
   for (const intent of INTENTS) {
      ...
      if (matches > 0) { console.log(STRICT MATCH IN  with  matches); scores[intent.id] = matches; }
   }
   ...
   if (!bestIntent && options.fallbackPrefix) {
      for (const intent of INTENTS) {
         ...
         for (const kw of intent.keywords) {
            ...
            const normKw = normalizeArabic(kw);
            // Left boundary strict, right boundary open (prefix match)
            // Prevent [^a-zA-Z0-9] from accidentally matching all Arabic letters in francoText
            const regex = new RegExp(`(?:^|\\s|[\\-\\*\\.,_]|[بفلك]|لل)${normKw}`, 'gi');
            const count1 = (normText.match(regex) || []).length;
            const count2 = (francoText.match(regex) || []).length;
            matches += Math.max(count1, count2);
         }
